...
## IO_SIGN
resource "azurerm_api_management_api_version_set" "io_backend_io_sign_api" {
  name                = format("%s-io-backend-io-sign-api", var.env_short)
  resource_group_name = module.apim.resource_group_name
  api_management_name = module.apim.name
  display_name        = "${local.apim_io_backend_api.display_name} - sign"
  versioning_scheme   = "Segment"
}

module "apim_io_backend_io_sign_api_v1" {
  source = "git::https://github.com/pagopa/azurerm.git//api_management_api?ref=v1.0.16"
  ...
  product_ids           = [module.apim_io_backend_product.product_id]
  subscription_required = local.apim_io_backend_api.subscription_required
  version_set_id        = azurerm_api_management_api_version_set.io_backend_io_sign_api.id
  api_version           = "v1"
  service_url           = local.apim_io_backend_api.service_url

  description  = "${local.apim_io_backend_api.description} - sign"
  display_name = "${local.apim_io_backend_api.display_name} - sign"
  path         = "${local.apim_io_backend_api.path}/sign"
  protocols    = ["https"]

  content_format = "swagger-json"
  content_value = templatefile("./api/io_backend/sign/v1/_swagger.json.tpl", {
    host = local.apim_hostname_api_app_internal # api-app.internal.io.pagopa.it
  })

  xml_content = file("./api/io_backend/sign/v1/_base_policy.xml")
}
##
