locals {
  ...
  function_subscriptionmigrations = {
    app_settings = {
      ...
      DB_HOST         = "${var.project}-subsmigrations-db-postgresql.postgres.database.azure.com"
      DB_PORT         = 5432
      DB_IDLE_TIMEOUT = 30000 // milliseconds
      DB_NAME         = "db"
      DB_SCHEMA       = "SelfcareIOSubscriptionMigrations"
      DB_TABLE        = "migrations"
      DB_USER         = "${data.azurerm_key_vault_secret.subscriptionmigrations_db_server_adm_username.value}@${var.project}-subsmigrations-db-postgresql.postgres.database.azure.com"
  ...
  function_devportalservicedata = {
    app_settings = {
      ...
      // connection to PostgresSQL
      DB_HOST         = format("%s.postgres.database.azure.com", format("%s-%s-db-postgresql", var.project, "devportalservicedata"))
      DB_PORT         = 5432
      DB_IDLE_TIMEOUT = 30000 // milliseconds
      DB_NAME         = "db"
      DB_SCHEMA       = "DeveloperPortalServiceData"
      DB_TABLE        = "services"
      DB_USER         = var.db_server_data.username
      DB_PASSWORD     = var.db_server_data.password
