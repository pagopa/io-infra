locals {
  issuer_func_name      = format("%s-issuer-func", local.project)
  issuer_func_container = "src"
  issuer_func_package   = format("https://%sst.blob.core.windows.net/%s/%s.zip", replace(local.issuer_func_name, "-", ""), local.issuer_func_container, var.io_sign_issuer_func.version)
}
...
module "io_sign_issuer_func" {
  ...
  app_settings = {
    ...
    WEBSITE_RUN_FROM_PACKAGE                        = local.issuer_func_package
