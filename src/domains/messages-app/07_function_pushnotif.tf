...
locals {
  function_push_notif = {
    app_settings_common = {
      ...
      NOTIFICATIONS_QUEUE_NAME                = azurerm_storage_queue.push_notifications_queue.name
      NOTIFICATIONS_STORAGE_CONNECTION_STRING = module.push_notifications_storage.primary_connection_string
...
module "push_notif_snet" {
  ...
  address_prefixes                               = [var.cidr_subnet_appmessages[count.index]] // TODO
  resource_group_name                            = azurerm_resource_group.push_notif_rg.name
