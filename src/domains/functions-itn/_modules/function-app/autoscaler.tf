resource "azurerm_monitor_autoscale_setting" "function_services_autoscale" {
  ...
  name                = format("%s-autoscale", module.function_services_dx[count.index].name) #TODO switch to dx ones after 
  resource_group_name = azurerm_resource_group.services_rg[count.index].name
  location            = var.location_itn
  target_resource_id  = module.function_services_dx[count.index].app_service_plan_id
  ...
  profile {
    ...
    rule {
      metric_trigger {
        ...
        metric_resource_id       = module.function_services_dx[count.index].id
    ...
    rule {
      metric_trigger {
        ...
        metric_resource_id       = module.function_services_dx[count.index].app_service_plan_id
