...
module "session_manager_weu_05" {
  source = "github.com/pagopa/terraform-azurerm-v3//app_service?ref=v8.28.1"

  # App service plan
  plan_type              = "internal"
  plan_name              = format("%s-session-manager-asp-05", local.common_project)
  zone_balancing_enabled = true
  sku_name               = var.session_manager_plan_sku_name

  # App service
  name                = "${local.app_name_weu}-05"
  resource_group_name = azurerm_resource_group.session_manager_rg_weu_02.name
  location            = var.location
...
module "session_manager_weu_staging_05" {
  source = "github.com/pagopa/terraform-azurerm-v3//app_service_slot?ref=v8.28.1"

  app_service_id   = module.session_manager_weu_05.id
  app_service_name = module.session_manager_weu_05.name
  ...
  subnet_id                     = module.session_manager_snet_05.id
  vnet_integration              = true
  public_network_access_enabled = false
...
module "pub_session_manager_staging_05" {
  ...
  principal_id    = module.session_manager_weu_staging_05.principal_id
  subscription_id = data.azurerm_subscription.current.subscription_id
