  ...
  url_path_map = {
    io-backend-path-based-rule = {
      ...
      path_rule = {
        ...
        session-manager = {
          paths                 = ["/api/auth/v1/*", "/api/auth/v2/*", "/api/sso/bpd/v1/user", "/api/sso/bpd/v2/user", "/api/sso/pagopa/v1/user", "/api/sso/pagopa/v2/user", "/api/sso/zendesk/v1/jwt", "/api/sso/zendesk/v2/jwt"]
          backend               = "platform-api-gateway"
          rewrite_rule_set_name = "rewrite-rule-set-api-app"
        },
